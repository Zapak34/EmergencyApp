...
public class DisasterRecyclerViewAdapter extends RecyclerView.Adapter<DisasterRecyclerViewAdapter.ViewHolder>{
    private Context context;
    private List<MainRecyclerViewRowClass> my_data;


    public DisasterRecyclerViewAdapter(Context context, List<MainRecyclerViewRowClass> my_data) {
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        holder.count =  DisasterFavorites.count(DisasterFavorites.class, "disaster_id = ?",temp);
        if(holder.count > 0){
        ...
        holder.ivFavorite.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                String[] temp = {String.valueOf(holder.alertID)};
                long countTemp =  DisasterFavorites.count(DisasterFavorites.class, "disaster_id = ?",temp);
                    if(holder.count == 0)
                    {
                        //querying list return empty, there is no record found matching the query.
                        DisasterFavorites disasterFavorites = new DisasterFavorites();
                        disasterFavorites.setDisasterID(holder.alertID);
                        disasterFavorites.setDisasterTitle(holder.title.getText().toString());
                        disasterFavorites.setDisasterContent(holder.content.getText().toString());
                        disasterFavorites.setDisasterImageURL(holder.image_url);
                        disasterFavorites.setDisasterPostedTime(holder.time_posted.getText().toString());
                        disasterFavorites.save();
                    ...
                    if(countTemp > 0)
                    {
                        ...
                        DisasterFavorites.deleteAll(DisasterFavorites.class, "disaster_id = ?", String.valueOf(holder.alertID));
                        holder.ivFavorite.setImageResource(R.mipmap.ic_star_off);
